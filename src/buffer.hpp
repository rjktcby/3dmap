...
public:
  ...
  void swap (buffer&);
  char* detach ();
  void attach (char *d, size_type s);
...
inline void buffer::attach (char *d, size_type s)
{
  if (free_)
    delete[] data_;
...
inline void buffer::drain (size_type s)
{
  if (s < size_)
  {
    std::memmove (data_, data_ + s, size_ - s);
    size_ -= s;
  }
}
